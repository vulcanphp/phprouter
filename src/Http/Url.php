...
class Url implements JsonSerializable
{
    ...
    /**
     * Get all Subdomains contain this Url
     * @return null|array 
     */
    public function subDomains(): ?array
    {
        preg_match_all('/(?:http[s]*\:\/\/)*(.*?)\.(?=[^\/]*\..{2,5})/i', $this->absoluteUrl(), $match);
        return isset($match[1]) && !empty($match[1]) ? $match[1] : null;
    }
    ...
    {
        return $this->setParams(array_merge($this->params(), $params));
    }
    ...
    public function pushParam(array $push = [], array $param = []): string
    {
        $params = [];
        ...
        return static::arrayToParams($params);
    }
    ...
    {
        return stripos(trim($this->originalUrl(), '/') . '/', trim($value, '/') . '/') !== false;
    }
    ...
    {
        $scheme = $this->scheme !== null ? $this->scheme . '://' : 'http://';
        $host   = $this->host ?? '';
